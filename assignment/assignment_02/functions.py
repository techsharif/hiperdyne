...
def repair_dataset(dataset, threshold, partition, ignore=.4):
	for column in list(dataset.columns.values):
		if len(set(dataset[column].tolist())) / len(dataset[column].tolist()) > ignore:
			dataset=dataset.drop(column,axis=1)
		elif len(set(dataset[column].tolist())) / len(dataset[column].tolist()) > threshold:
...
def entropy(target_col):
    ...
    data = [(-counts[i]/np.sum(counts))*np.log2(counts[i]/np.sum(counts)) for i in range(len(elements))]
    entropy = np.sum(data)
    return entropy
...
def train_test_split(dataset, total_data):
    training_data = dataset.iloc[:total_data].reset_index(drop=True)#We drop the index respectively relabel the index
